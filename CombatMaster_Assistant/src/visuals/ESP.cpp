...
namespace ESP {
    ...
    void DrawCornerBox(float x, float y, float w, float h, ImU32 color, float thickness) {
        ...
        float line_w = w / 4.0f; float line_h = h / 4.0f;
        ImDrawList* drawList = ImGui::GetBackgroundDrawList();
    ...
    void DrawTextCentered(const std::string& text, float x, float y, ImU32 color) {
        ...
        DrawUtils::DrawGlowText(ImGui::GetBackgroundDrawList(), ImGui::GetFont(), ImGui::GetFontSize(), 
            ImVec2(x - size.x / 2, y), color, text.c_str(), 0.5f);
    }
